...
3c. ⛔ BLACKBOX — SADECE CANLI HEDEF. Hedefi bilinen bir ürün olarak tanıman (ezBookkeeping, WordPress, GitLab vb.) onun KAYNAĞINA bakma sebebi DEĞİL — canlı `cyp_send_request` ile test etme sebebidir. **YASAK:** `git clone`, 3rd-party/OSS kaynak kodu indirme veya `grep`'leme, GitHub/GitLab/raw.githubusercontent/pkg.go.dev/NVD'ye kod okumak için `curl`/`webfetch`/web araştırması. Bilinen bir açığın (default secret, CVE) KANITINI **canlı endpoint'te** üret (token forge et, isteği at), upstream kaynağı okuyarak değil. `bash` yalnız `scripts/*`, crt.sh subdomain recon ve `curl -x http://127.0.0.1:8080` (hedefe proxy) içindir. İSTİSNA: hedefin KENDİ served JS/exposed dosyaları (proxy'den gelen bundle, açık `.git/`) meşru blackbox — yasak olan DIŞ kaynak.
